...
def render_stats_and_streaks(puzzle_type, stats, streaks):
    """Render display templates and produce *-stats.html"""

    filename = f"{puzzle_type}-stats.html"
...
def format_time(total_seconds):
    """Convert time from seconds to H:m:s string where H is excluded if less than an hour.
    Empty times have a special representation
    """
...
def get_bar_size(current_time, fastest_time, slowest_time):
    """Calculate bar height and width bsed on fastest and slowest times"""

    scale_factor = (current_time - fastest_time) / (slowest_time - fastest_time)

    # todo? magic numbers from
    return {"height": 70 + 205 * scale_factor, "width": 80 + 240 * scale_factor}


def prep_stats_and_streaks(stats, streaks):
    """Convert stats and streaks to formatted data to populate template"""
    ...
    for day in stats["stats_by_day"]:
        ...
        latest_time_size = get_bar_size(
            day["this_weeks_time"], fastest_time, slowest_time
        )
        best_time_size = get_bar_size(day["best_time"], fastest_time, slowest_time)
        average_time_size = get_bar_size(day["avg_time"], fastest_time, slowest_time)
        best_date_text = (
            date.fromisoformat(day["best_date"]).strftime("%Y-%m-%d")
            if day["best_date"] != 0
            else ""
        )
